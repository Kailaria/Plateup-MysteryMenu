...
{
    public class MysteryHomemadeStrawberryIceCreamDish : GenericMysteryDish
    {
        protected override string NameTag => "Sundae/Ice Cream - Homemade Strawberry";
        ...
        {
            { Locale.English,
                "<color=yellow>Requires ingredient:</color> Milk, Sugar, Strawberry\n" + 
                "Combine Milk and Sugar into a Mixing Bowl, then freeze it. After it has frozen, " +
                "combine with a Strawberry, then knead to make Strawberry Ice Cream. Provides 5 portions." }
        };
        public override List<(Locale, UnlockInfo)> InfoList => new()
        {
            (Locale.English, new UnlockInfo()
            {
                Name = "Mystery - Homemade Strawberry Ice Cream",
                Description = "Substitutes Strawberry Ice Cream for its homemade variant.",
        ...
        public override HashSet<SubstitutionIngredientSet> SubstitutionIngredientSets => new()
        {
            new SubstitutionIngredientSet(
                GDOUtils.GetExistingGDO(ItemReferences.IceCreamStrawberry) as Item,
                new HashSet<Item>()
                {
                    GDOUtils.GetExistingGDO(ItemReferences.Milk) as Item,
                    GDOUtils.GetExistingGDO(References.Strawberry) as Item,
